...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        let game = games[i];
        console.log(`Game ${i + 1}: ${game.name}`);
...
let newtotalRaised = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);
const summaryMessage = `
  A total of $${newtotalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} games. 
